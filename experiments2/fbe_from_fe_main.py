...
from experiments2.final_statistics import val_all_methods_best_configuration, get_best_hyperparameters_from_keys
...
def get_best_params(fs_name, cls_name, best_hyperparams):
    best_hyperparams_copy = [x for x in best_hyperparams if x['FS']["NAME"]==fs_name]
    best_hyperparams_copy = [x for x in best_hyperparams_copy if x['CLS']["NAME"]==cls_name]
    best_hyperparams_copy = best_hyperparams_copy[0]
    return best_hyperparams_copy


def setup_config(trial, res_filename, cls_name, metric):
    ...
    best_conf = get_best_params("ChronOMP", cls_name, all_best_hyperparams)
    fs_conf = best_conf["FS"]
    cls_conf = best_conf["CLS"]
    
    fs_conf["CONFIG"]["config"]["significance_threshold_backward"] = trial.suggest_float("significance_threshold_backward", 1e-20, 0.1, log=True)
    fs_conf["CONFIG"]["config"]["method_backward"] = trial.suggest_categorical("method_backward",["f-test", "wald-test", "lr-test"])
...
def generate_optuna_objective_function(fs_name, cls_name, proxy_cls_name, dataset_setup, objective="R2"):
    ...
    def optuna_objective_function(trial):
        ...
        config_file = setup_config(trial, res_filename, cls_name, metric=objective)
...
def full_experiment(dataset, cls_name, experiment_identifier, seed=0):
    ...
    for i,filename in enumerate(filelist):
        ...
        for target in target_set:
            ...
            fs_space = baselines.feature_selection.generate_optuna_search_space("BackwardChronOMP")
            fs_space = {k:fs_space[k] for k in ["significance_threshold_backward", "method_backward"]}
        
            # GridSampler launch
            studylength = np.prod([len(x) for _,x in fs_space.items()])
            if first_evaluation_flag:
                print("\t\tNumber of configurations to be evaluated:",studylength)
            objective, results = generate_optuna_objective_function(cls_name, dataset_setup)
...
if __name__=="__main__":
    _, data, model, experiment_identifier = sys.argv
    print(data)
    full_experiment(data, model, experiment_identifier)
