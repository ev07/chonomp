...
def generate_optuna_objective_function(fs_name, cls_name, dataset_setup, objective="R2"):
    ...
    def optuna_objective_function(trial):
        ...
        #here insert the different configurations necessary for the different dataset parameters
        #since we are doing a grid search and not optimizing, it does not matter what we send back.
        sr_ = [0.1, 0.25, 0.5, 0.75, 1., 1., 1., 1., 1., 1.]
        cr_ = [1., 1., 1., 1., 1., 0.0, 0.25, 0.5, 0.75, 1.]
    
        for sr,cr in zip(sr_, cr_):  # control the sample and covariate ratio
            dataset_setup["DATASET"]["SAMPLE_RATIO"] = sr
            dataset_setup["DATASET"]["COVARIATE_RATIO"] = cr
